...
if ($action === 'trade_contract' && isset($_GET['trade_id'])) {
    ...
    $conn = connectDB();
    $stmt = $conn->prepare("SELECT t.*, i.description, i.make_model, i.asking_price, c.name AS consignor_name, c.phone AS consignor_phone FROM trades t JOIN items i ON t.item_id = i.id LEFT JOIN consignors c ON i.consignor_id = c.id WHERE t.id = ?");
    ...
    if ($result && $result->num_rows > 0) {
        ...
        <strong>Model:</strong> " . htmlspecialchars($trade['make_model']) . "<br> <strong>Listed Value:</strong> \${$item_value}</p>
        ...
        <h4>Terms</h4>
        <p>" . nl2br(htmlspecialchars($trade['trade_notes'])) . "</p>

<div class='terms'>
    
    <p>Back2Work Equipment is not responsible for any defects, damages, or disputes regarding the traded item. All trades are final and are made between the original consignor and the trader. 
    We do not inspect, guarantee, or warrant the functionality, safety, or condition of any equipment.</p>

    <p>All trades are final. No returns, refunds, or exchanges will be accepted for any reason. Back2Work Equipment is not responsible for defects, title issues, or performance of traded items.
    All trades are made “as-is” and “where-is,” without warranties of any kind.</p>
    ...
    <p>The Consignor affirms they are the rightful owner of the item and that it is free from any liens, security interests, or claims. The Consignor certifies that the information provided above is accurate to the best of their knowledge.
    No formal title exists for the item unless otherwise disclosed. The buyer/trader agrees to indemnify Back2Work Equipment against any ownership disputes or claims.</p>
...
<table>
 <tr>
  ...
  <tr>
    <td><strong>Trader Signature/Date:</strong><br></td>
  </tr>
   <tr>
    <td><br></td>
  ...
  <tr>
    <td><strong>Back2Work Equipment Representative Signature/Date:</strong><br><br></td>
  </tr>
</table>
...
// (then your other logic continues...)
...
function generateInvoice($sale_id) {
    ...
    if ($result && $result->num_rows > 0) {
        ...
        if (!empty($data['delivery_method']) && strtolower($data['delivery_method']) === 'delivery') {
            $invoice .= "<hr>";
            $invoice .= "<p><strong>Delivery Method:</strong> Delivery</p>";
...
function recordSale($item_id, $sale_price, $buyer_name, $buyer_contact, $payment_method, $credit_applied = 0) {
    $conn = connectDB();
    // STEP 1 - Load item info (LEFT JOIN to avoid missing house inventory)
    ...
    // STEP 2 - Check if consignor has multiple active items (only if consignor exists)
    $multiple_items = 0;
...
// Updated SQL with license_file column
...
$stmt_sale->bind_param(
    "isddddsssdds",  // added one more 's' at the end for license_file
    $item_id, 
    $today, 
    $sale_price, 
    $comm_rate, 
    ...
    if ($stmt_sale->execute()) {
        ...
        // Update item status
        $sql_update = "UPDATE items SET status = 'sold' WHERE id = ?";
        $stmt_update = $conn->prepare($sql_update);
        $stmt_update->bind_param("i", $item_id);
        $stmt_update->execute();
        // Update consignor as repeat seller (ONLY if consignor exists)
        if (!empty($item['consignor_id'])) {
            $sql_repeat = "UPDATE consignors SET repeat_seller = 1 WHERE id = ?";
            $stmt_repeat = $conn->prepare($sql_repeat);
            $stmt_repeat->bind_param("i", $item['consignor_id']);
            $stmt_repeat->execute();
        }
        // STEP 5 - Deduct used store credit (insert into credits_redeemed table)
        if ($credit_applied > 0) {
            $sql_credit = "INSERT INTO credits_redeemed (customer_name, amount, date_redeemed) VALUES (?, ?, ?)";
...
// Function to display a form to add a new item
function displayAddItemForm() {
   ...
   // $output .= "<option value='house_inventory'>House Inventory</option>";
    while ($row = $result->fetch_assoc()) {
    ...
    $output .= "</div>";
    $output .= "<div class='form-group'>";
    $output .= "<label for='condition_desc'>Condition:</label>";
    $output .= "<textarea name='condition_desc' id='condition_desc' class='form-control'></textarea>";
    $output .= "</div>";
    $output .= "<div class='form-group'>";
...
$output .= "</select>";
$output .= "</div>";
// Add this for trade authorization
$output .= "<div class='form-group'>";
...
case 'generate_invoice':
    if (isset($_GET['sale_id'])) {
        ...
        $sql = "SELECT 
                    s.id, s.sale_date, s.sale_price, s.sales_tax, s.commission_amount, s.commission_rate, 
                    s.credit_applied, s.delivery_method, s.delivery_fee, s.mileage, s.scheduled_time,
                    s.buyer_name, s.buyer_contact, s.buyer_phone, s.buyer_address,
                    s.payment_method, s.license_file,
                    i.description, i.make_model, i.consignor_id, i.owned_by_company,
        ...
        if ($result && $result->num_rows > 0) {
            ...
            $content .= "<p>&nbsp;</p><h2>Invoice #{$sale['id']}</h2>";
            ...
            if (!empty($sale['delivery_method']) && $sale['delivery_method'] === 'delivery') {
                $mileage = floatval($sale['mileage']);
                $content .= "<p><strong>Delivery Method:</strong> Delivery</p>";
            ...
            $credit_applied = floatval($sale['credit_applied']);
            $delivery_fee = floatval($sale['delivery_fee']);
            $total_before_credit = $sale_price + $sales_tax + $delivery_fee;
        ...
        case 'lead_tracker':
            ...
            case 'inventory':
                if ($action === 'start_trade' && isset($_GET['item_id'])) {
                    // Your Start Trade code (form generation)
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    [tradeValue, brokerFee, cashDifference, overrideCheckbox].forEach(el => {
...
case 'sale_details':
    if (isset($_GET['sale_id'])) {
        ...
        if ($result && $result->num_rows > 0) {
            ...
            $delivery_fee = floatval($sale['delivery_fee']);
            $mileage = floatval($sale['mileage']);
            $scheduled = !empty($sale['scheduled_time']) ? htmlspecialchars($sale['scheduled_time']) : 'N/A';
...
case 'commission_rates':
    ...
    fputcsv($output, ['Quarter', 'Sales Total', 'Delivery Fees', 'Commission Total']);
    ...
    $content .= "<tr><td><strong>ALL-TIME Total Delivery Fees</strong></td><td>$" . number_format($tax_data['total_delivery'], 2) . "</td></tr>";
    ...
    $content .= "<div class='table-responsive'>";
    $content .= "<table class='table table-bordered'>";
    $content .= "<thead><tr><th>Quarter</th><th>Sales Total</th><th>Delivery Fees</th><th>Commission</th></tr></thead><tbody>";
...
if ($action == 'process_sale' && isset($_GET['item_id']) && $_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sale_price = floatval($_POST['sale_price'] ?? 0);
    $delivery_fee = ($_POST['delivery_method'] === 'delivery') ? 50.00 : 0.00;
