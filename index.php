...
// Get page and action parameters with default values
...
if ($environment === 'development') {
    error_log("DEBUG: Page = $page, Action = $action");
}
...
// ===== [INVENTORY SECTION] =====
// Add/edit items, consignor views, house inventory, etc.
// ====================[ INVENTORY | function checkInventoryAging() ]====================
/**
 * Returns an array of active items nearing 30, 60, or 120 days on the lot.
 * Used to notify staff or flag aging inventory.
 */
...
// Reusable JS, preview logic, formatting

// ====================[ UTILITIES | function displayDashboard() ]====================
/**
 * Generates the dashboard HTML for the homepage.
 * Displays counts for inventory, aging items, and sales activity (30 & 120 days).
 * @return string HTML output for dashboard
 */
$content .= "<div style='padding: 50px; color: red;'>TEST CONTENT</div>";
 ...
 function displayDashboard() {
    ...
    try {
        ...
        $sql = "SELECT COUNT(*) as count FROM items WHERE status = 'active'";
        $result = $conn->query($sql);
        if (!$result) throw new Exception("Query failed: " . $conn->error);
        $active_count = $result->fetch_assoc()['count'];
        
        // Get 30-day aging items count
        $sql = "SELECT COUNT(*) as count FROM items WHERE status = 'active' AND 
                DATEDIFF(CURDATE(), date_received) >= 30 AND DATEDIFF(CURDATE(), date_received) < 60";
        $result = $conn->query($sql);
        if (!$result) throw new Exception("Query failed: " . $conn->error);
        $items_30days_count = $result->fetch_assoc()['count'];
        
        // Get 60-day aging items count
        $sql = "SELECT COUNT(*) as count FROM items WHERE status = 'active' AND 
                DATEDIFF(CURDATE(), date_received) >= 60 AND DATEDIFF(CURDATE(), date_received) < 120";
        $result = $conn->query($sql);
        if (!$result) throw new Exception("Query failed: " . $conn->error);
        $items_60days_count = $result->fetch_assoc()['count'];
        
        // Get 120-day aging items count
        $sql = "SELECT COUNT(*) as count FROM items WHERE status = 'active' AND 
                DATEDIFF(CURDATE(), date_received) >= 120";
        $result = $conn->query($sql);
        if (!$result) throw new Exception("Query failed: " . $conn->error);
        $items_120days_count = $result->fetch_assoc()['count'];
        
        // Get 30-day sales data
        $sql = "SELECT COUNT(*) as sales_count, SUM(commission_amount) as total_commission 
                FROM sales WHERE DATEDIFF(CURDATE(), sale_date) <= 30";
        $result = $conn->query($sql);
        if (!$result) throw new Exception("Query failed: " . $conn->error);
        $sales_data = $result->fetch_assoc();
        if ($sales_data['total_commission'] === null) $sales_data['total_commission'] = 0;
        
        // Get 120-day sales data
        $sql = "SELECT COUNT(*) as sales_count_120, SUM(commission_amount) as total_commission_120 
                FROM sales WHERE DATEDIFF(CURDATE(), sale_date) <= 120";
        $result = $conn->query($sql);
        if (!$result) throw new Exception("Query failed: " . $conn->error);
        $sales_data_120 = $result->fetch_assoc();
        if ($sales_data_120['total_commission_120'] === null) $sales_data_120['total_commission_120'] = 0;
        ...
        $output .= "<div class='stat-box danger'><h3>120-Day Alerts</h3><p class='stat-count'>{$items_120days_count}</p></div>";
        ...
        $output .= "<div class='stat-box success'><h3>120-Day Sales</h3><p class='stat-count'>{$sales_data_120['sales_count_120']}</p></div>";
        $output .= "<div class='stat-box success'><h3>120-Day Commission</h3><p class='stat-count'>$" . number_format($sales_data_120['total_commission_120'], 2) . "</p></div>";
        $output .= "</div>";
        
        // Add quick action buttons
        $output .= "<div class='quick-actions mt-4'>";
        $output .= "<h3>Quick Actions</h3>";
        $output .= "<div class='btn-group'>";
        $output .= "<a href='?page=inventory&action=add' class='btn btn-primary'>Add New Item</a> ";
        $output .= "<a href='?page=consignors&action=add' class='btn btn-secondary'>New Consignor</a> ";
        $output .= "<a href='?page=rentals&action=create' class='btn btn-info'>Create Rental</a> ";
        $output .= "<a href='?page=sales_history&action=add' class='btn btn-success'>Record Sale</a>";
        $output .= "</div>";
        $output .= "</div>";
        
    } catch (Exception $e) {
        $output .= "<div class='alert alert-danger'>Error generating dashboard: " . htmlspecialchars($e->getMessage()) . "</div>";
    }
    
    $output .= "<div style='padding:20px; background:#eee;'>DEBUG: Returning from dashboard</div>";
...
// ====================[ UTILITIES | function handleRequest() ]====================
/**
 * Core request handler for routing actions and pages.
 * Initializes the database, evaluates $_GET['page'] and $_GET['action'],
 * and sets overrides for editing scenarios.
 * @return string Page content HTML
 */
function handleRequest() {
    setupDatabase(); // Ensure database and tables are initialized
    $content = "";
    ...
    // Optional debug markers (can be removed in production)
    // $content .= "<div style='color: green;'>Entered handleRequest()</div>";
    // $content .= "<div style='color: blue;'>Page: $page | Action: $action</div>";

    // Override page if editing
...
case 'add_item':
    ...
    case 'lead_tracker':
        ...
        case 'trade':
            $conn = connectDB();
            $content .= "<h3>Trade Log</h3><table border='1'><tr><th>Item</th><th>Traded For</th><th>Broker Fee</th><th>Rep</th><th>Date</th></tr>";
            $result = $conn->query("SELECT * FROM trades ORDER BY created_at DESC");
            if ($result && $result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    $content .= "<tr><td>{$row['item']}</td><td>{$row['trade_for']}</td><td>{$row['broker_fee']}%</td><td>{$row['rep']}</td><td>{$row['created_at']}</td></tr>";
                }
            } else {
                $content .= "<tr><td colspan='5'>No trades found</td></tr>";
            }
            $content .= "</table>";
            
            $content .= "<form method='post' action='?page=trade'>
                <label>Item: <input type='text' name='item'></label><br>
                <label>Trade For: <input type='text' name='trade_for'></label><br>
                <label>Broker Fee (%): <input type='number' name='broker_fee' step='0.01'></label><br>
                <label>Intake Rep: <input type='text' name='rep'></label><br>
                <input type='submit' name='submit_trade' value='Record Trade'>
            </form><hr>";
        
            if (isset($_POST['submit_trade'])) {
                $stmt = $conn->prepare("INSERT INTO trades (item, trade_for, broker_fee, rep, created_at) VALUES (?, ?, ?, ?, NOW())");
                ...
                $stmt->execute();
                $content .= "<p style='color:green;'>Trade recorded.</p>";
            }
            $conn->close();
            break;
...
case 'generate_invoice':
    if (isset($_GET['sale_id'])) {
        ...
        $conn = connectDB();
        $sql = "SELECT s.*, i.description, i.make_model, i.consignor_id, i.owned_by_company, c.name AS consignor_name
                FROM sales s
        ...
        if ($result && $result->num_rows > 0) {
            ...
            $sales_tax = floatval($sale['sales_tax']);
            $commission_amount = floatval($sale['commission_amount']);
            $commission_rate = floatval($sale['commission_rate']);
            $credit_applied = floatval($sale['credit_applied'] ?? 0);
            $total_before_credit = $sale_price + $sales_tax;
            $total_paid = $total_before_credit - $credit_applied;
            ...
            $content .= "<style>
                ...
                .invoice-box h2, .invoice-box p { margin: 0 0 10px; }
                .invoice-box .header { background-color: #333; color: #fff; padding: 20px; text-align: center; border-radius: 4px; }
                .invoice-box hr { margin: 20px 0; }
            </style>";
            $content .= "<div class='invoice-box'>";
            $content .= "<div class='header'>
                <h1 style='margin: 0; font-size: 28px;'>BACK 2 WORK EQUIPMENT</h1>
                <p style='margin: 0; font-size: 16px;'>Buy - Sell - Rent - Trade</p>
                <p style='margin: 0;'>www.back2workequipment.com</p>
                <p style='margin: 0;'>10460 US Hwy 79 E, Oakwood, TX 75855 | (903) 721-5544</p>
            </div>";
            $content .= "<p>&nbsp;</p><h2>Invoice #{$sale_id}</h2>";
            ...
            $content .= "<p><strong>Contact:</strong> " . htmlspecialchars($sale['buyer_contact']) . "</p>";

            // Delivery / Pickup Info
            if (!empty($sale['delivery_method'])) {
                $method_label = ucfirst($sale['delivery_method']);
                $mileage = $sale['mileage'] ?? 0;
                $eligible = ($mileage <= 30 && ($sale_price - $commission_amount) >= 75 && $method_label !== '') 
                    ? "✓ Eligible" 
                    : "✗ Not Eligible";
                $content .= "<p><strong>Delivery Method:</strong> {$method_label}</p>";
                $content .= "<p><strong>Mileage (one-way):</strong> " . floatval($mileage) . " miles</p>";
                $content .= "<p><strong>Pickup/Delivery Eligibility:</strong> {$eligible}</p>";
                if (!empty($sale['scheduled_time'])) {
                    $scheduled = date('m/d/Y g:i A', strtotime($sale['scheduled_time']));
                    $content .= "<p><strong>Scheduled Time:</strong> {$scheduled}</p>";
                }
                if (!empty($sale['buyer_address'])) {
                    $content .= "<p><strong>Address:</strong> " . nl2br(htmlspecialchars($sale['buyer_address'])) . "</p>";
                }
                if (!empty($sale['buyer_phone'])) {
            ...
            if ($credit_applied > 0) {
                $content .= "<p><strong>Store Credit Applied:</strong> -$" . number_format($credit_applied, 2) . "</p>";
            }
            $content .= "<p><strong>Sales Tax (8.25%):</strong> $" . number_format($sales_tax, 2) . "</p>";
            $content .= "<p><strong>Total Paid:</strong> $" . number_format($total_paid, 2) . "</p>";
            $content .= "<p><strong>Payment Method:</strong> " . htmlspecialchars($sale['payment_method']) . "</p>";
            $content .= "<hr>";
            if ($commission_rate === 0) {
                $content .= "<p><strong>Commission:</strong> $" . number_format($commission_amount, 2) . " (Flat Fee)</p>";
            } else {
                $content .= "<p><strong>Commission (" . number_format($commission_rate, 1) . "%):</strong> $" . number_format($commission_amount, 2) . "</p>";
            }
            $content .= "<p><strong>Consignor:</strong> " . htmlspecialchars($sale['consignor_name']) . "</p>";
            $content .= "<p><strong>Amount to Consignor:</strong> $" . number_format($sale_price - $commission_amount, 2) . "</p>";
            $content .= "<hr>";
            $content .= "<p><em>Thank you for your business!</em></p>";
            $content .= "<p><strong>All Sales Final.</strong> No refunds, returns, or exchanges unless otherwise agreed in writing.</p>";
            $content .= "<p style='margin-top:40px;'><strong>Customer Signature:</strong> ____________________________________________</p>";
            $content .= "<p>By signing, customer agrees to the All Sales Final policy stated above.</p>";
            $content .= "<div class='no-print' style='margin-top:30px;'>
                <button onclick='window.print()' class='btn btn-primary'>Print Invoice</button>
                <a href='?page=dashboard' class='btn btn-secondary'>Back</a>
            </div>";
            $content .= "</div>"; // close invoice-box
        } else {
            $content .= "<p class='text-danger'>Sale not found.</p>";
        }
        $conn->close();
    } else {
        $content .= "<p class='text-danger'>No sale ID provided.</p>";
    }
    break;
...
case 'broker_earnings':
    $conn = connectDB();
    $sql = "
        SELECT 
            DATE_FORMAT(created_at, '%Y-%m') AS month,
            SUM(trade_value * (broker_fee / 100)) AS earnings,
            COUNT(*) AS trades
        FROM trades
        GROUP BY month
        ORDER BY month DESC
    ";
    $result = $conn->query($sql);
    $content .= "<h2 class='mt-4'>Broker Earnings Summary (Monthly)</h2>";
    if ($result && $result->num_rows > 0) {
        $content .= "<div class='table-responsive'>
        ...
        $content .= "<p class='text-muted'>No trade earnings data available.</p>";
    }
    $conn->close();
    break;
...
// Pay Consignor (if not house inventory)
...
case 'sale_details':
    if (isset($_GET['sale_id'])) {
        ...
        if ($result && $result->num_rows > 0) {
            ...
            $content .= "<p><strong>Phone:</strong> " . htmlspecialchars($sale['buyer_phone']) . "</p>";
            $content .= "<p><strong>Address:</strong> " . nl2br(htmlspecialchars($sale['buyer_address'])) . "</p>";
            $content .= "<p><strong>Delivery Method:</strong> " . ucfirst($sale['delivery_method']) . "</p>";
            $content .= "<p><strong>Mileage (one-way):</strong> " . floatval($sale['mileage']) . " miles</p>";
            $content .= "<p><strong>Scheduled:</strong> " . htmlspecialchars($sale['scheduled_time']) . "</p>";

            $profit = $sale['sale_price'] - $sale['commission_amount'];
            $eligible = ($sale['mileage'] <= 30 && $profit >= 75) ? "✅ Eligible" : "❌ Not Eligible";
            $content .= "<p><strong>Eligibility:</strong> {$eligible}</p>";
        ...
        case 'edit_sale':
            if (isset($_GET['sale_id'])) {
                ...
                if ($result && $result->num_rows > 0) {
                    ...
                    $content .= "<h2 class='mt-4'>Edit Sale Delivery Info</h2>";
        
                    // ? Show customer and item info
                    $content .= "<div class='card mb-3'><div class='card-body'>";
                    $content .= "<p><strong>Buyer:</strong> " . htmlspecialchars($sale['buyer_name']) . "</p>";
                    $content .= "<p><strong>Item:</strong> " . htmlspecialchars($sale['description']) . " (" . htmlspecialchars($sale['make_model']) . ")</p>";
                    $content .= "</div></div>";
                ...
                $content .= "<div class='form-group'>
                    ...
                    <input type='text' class='form-control' name='buyer_phone' value='" . htmlspecialchars($sale['buyer_phone']) . "' required>
                </div>";
    
                $content .= "<div class='form-group'>
                    <label for='buyer_address'>Buyer Address:</label>
                    <textarea class='form-control' name='buyer_address' rows='2' required>" . htmlspecialchars($sale['buyer_address']) . "</textarea>
                </div>";
                ...
                $content .= "<div class='form-group'>
                    ...
                    <select class='form-control' name='delivery_method'>
                        ...
                        <option value='delivery'" . ($sale['delivery_method'] == 'delivery' ? " selected" : "") . ">Delivery</option>
                    </select>
                </div>";
    
                $content .= "<div class='form-group'>
                    <label for='mileage'>Mileage (one way):</label>
                    <input type='number' step='0.1' class='form-control' name='mileage' id='mileage'>
                </div>";
    
                $content .= "<div class='form-group'>
                    <label for='scheduled_time'>Scheduled Pickup/Delivery Time:</label>
                    <input type='datetime-local' class='form-control' name='scheduled_time' value='" . date('Y-m-d\TH:i', strtotime($sale['scheduled_time'])) . "'>
                </div>";
...
case 'commission_rates':
    ...
    case 'download_tax_report_csv':
        ...
        // Get all-time totals
        $sql = "SELECT SUM(sale_price) AS total_sales, SUM(commission_amount) AS total_commission FROM sales";
        $result = $conn->query($sql);
        $tax_data = $result->fetch_assoc();
    
        // Get this year's totals
        $sql_year = "SELECT SUM(sale_price) AS total_sales_year, SUM(commission_amount) AS total_commission_year 
                     FROM sales 
        ...
        foreach ($quarters as $quarter => $dates) {
            ...
            $sql_q = "SELECT SUM(sale_price) AS total_sales, SUM(commission_amount) AS total_commission 
                      FROM sales 
        ...
        fputcsv($output, ['Tax Report Summary']);
        fputcsv($output, ['Description', 'Amount']);
        fputcsv($output, ['ALL-TIME Total Sales', number_format($tax_data['total_sales'], 2)]);
        fputcsv($output, ['ALL-TIME Total Commission Earned', number_format($tax_data['total_commission'], 2)]);
        fputcsv($output, [date('Y') . ' Total Sales', number_format($tax_data_year['total_sales_year'], 2)]);
        fputcsv($output, [date('Y') . ' Total Commission Earned', number_format($tax_data_year['total_commission_year'], 2)]);
    
        fputcsv($output, []);
        fputcsv($output, ['Quarterly Sales Report']);
        fputcsv($output, ['Quarter', 'Sales Total', 'Commission Total']);
    
        foreach ($quarter_data as $quarter => $data) {
            fputcsv($output, [$quarter, number_format($data['total_sales'], 2), number_format($data['total_commission'], 2)]);
        }
    ...
    case 'tax_report':
        ...
        $content .= "<thead><tr><th>Description</th><th>Amount</th></tr></thead>";
        $content .= "<tbody>";
        $content .= "<tr><td><strong>ALL-TIME Total Sales</strong></td><td>$" . number_format($tax_data['total_sales'], 2) . "</td></tr>";
        $content .= "<tr><td><strong>ALL-TIME Total Commission Earned</strong></td><td>$" . number_format($tax_data['total_commission'], 2) . "</td></tr>";
        $content .= "<tr><td><strong>" . date('Y') . " Total Sales</strong></td><td>$" . number_format($tax_data_year['total_sales_year'], 2) . "</td></tr>";
        $content .= "<tr><td><strong>" . date('Y') . " Total Commission Earned</strong></td><td>$" . number_format($tax_data_year['total_commission_year'], 2) . "</td></tr>";
        $content .= "</tbody></table>";
        $content .= "</div>";
    
        // Output Quarterly Report
        $content .= "<h2 class='mt-5'>Quarterly Sales Tax Report (" . date('Y') . ")</h2>";
        $content .= "<div class='table-responsive'>";
        $content .= "<table class='table table-bordered'>";
        $content .= "<thead><tr><th>Quarter</th><th>Sales Total</th><th>Commission Total</th></tr></thead>";
        $content .= "<tbody>";
...
case 'create_rental':
    ...
    if ($item_id) {
        ...
        if ($result->num_rows > 0) {
            ...
            $content .= "<div class='form-group'><label>ID / License #</label><input type='text' name='renter_id_number' id='renter_id_number' class='form-control'>
</div>";
            ...
            $content .= "<div class='text-right mt-3'><a href='?action=print_return_receipt&rental_id={$item_id}' class='btn btn-outline-secondary'>Print Return Receipt</a></div>";
...
function updateRentalPreview() {
    ...
    html += "<p><strong>Delivery Required:</strong> " + (delivery ? "Yes" : "No") + "</p>";
    if (deliveryFee > 0) {
            ...
            case 'edit_item':
                if (isset($_GET['item_id'])) {
                    ...
                    if ($item) {
                        ...
                        $content .= "<div class='form-group'>";
                        $content .= "<label>Condition:</label>";
                        $content .= "<textarea name='condition_desc' class='form-control'>" . htmlspecialchars($item['condition_desc']) . "</textarea>";
                        $content .= "</div>";
                        ...
                        $content .= "<select name='consignor_id' class='form-control'>";
    ...
    if (isset($_GET['consignor_id']) && is_numeric($_GET['consignor_id'])) {
        ...
        if ($result && $result->num_rows > 0) {
            $content .= "<h4 class='mt-4'>Abandonment History</h4>
                         ...
                         <thead><tr><th>Item</th><th>Abandonment Date</th><th>Reason</th></tr></thead><tbody>";
            while ($row = $result->fetch_assoc()) {
                $content .= "<tr>
                                <td>" . htmlspecialchars($row['description']) . "</td>
                                <td>" . date('Y-m-d', strtotime($row['abandonment_date'])) . "</td>
        ...
        $stmt_sales = $conn->prepare("
            SELECT 
                s.sale_date, s.sale_price, s.commission_amount, s.sales_tax,
                s.buyer_name, s.payment_method, s.consignor_paid, s.license_file,
                i.description
            FROM sales s
        ...
        <?php if ($consignor): ?>
            ...
            <?php if (!empty($consignor['agreement_file'])): ?>
                ...
                <p><strong>Signed Agreement:</strong>
                    <a href="<?= $file_path ?>" target="_blank" <?= $force_download ? 'download' : '' ?> class="btn btn-sm btn-outline-primary"><?= $label ?></a>
                </p>
            <?php endif; ?>

            <p><strong>Authorized for Abandonment Charge:</strong>
                <?= $consignor['abandonment_approved'] ? '<span class="text-success">Yes</span>' : '<span class="text-danger">No</span>' ?>
            </p>
                    ...
                    <tbody>
                        <?php while ($sale = $sales_result->fetch_assoc()): ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php if (!empty($sale['license_file'])): ?>
                                        <a href="<?= htmlspecialchars($sale['license_file']) ?>" class="btn btn-sm btn-secondary" target="_blank">📎 View</a>
                                    <?php else: ?>
...
// ====================[ INVENTORY | function displayAddConsignorForm() ]====================
/**
 * Displays the HTML form to add a new consignor.
 * Includes fields for name, contact info, and address.
 * Also offers a link to print a blank agreement form.
...
function displayAddConsignorForm() {
    ...
    $output .= "<div class='text-right mt-3'>
        <a href='?action=generate_blank_agreement' target='_blank' class='btn btn-sm btn-warning ml-2'>
            Print Blank Agreement
        </a>
    </div>";
...
// ====================[ INVENTORY | function displayAddItemForm() ]====================
/**
 * Displays the HTML form to add a new item to inventory.
 * Includes: consignor select, pricing, rental/trade flags, pickup/delivery options.
 * @return string HTML markup for the form
 */
function displayAddItemForm() {
    $conn = connectDB();

    // Default empty item and fetch consignor list
    $item = [];
    $item['consignor_id'] = 0;

    $sql = "SELECT id, name FROM consignors ORDER BY (name = 'House Inventory') DESC, name";
    $result = $conn->query($sql);
    ...
    $output .= "<div class='form-group'>
        ...
        <select name='consignor_id' id='consignor_id' required class='form-control'>
    ...
    // Core item details
    $output .= "<div class='form-group'><label for='description'>Description:</label>
        <input type='text' name='description' id='description' required class='form-control'></div>";

    $output .= "<div class='form-group'><label for='make_model'>Make/Model:</label>
        <input type='text' name='make_model' id='make_model' class='form-control'></div>";

    $output .= "<div class='form-group'><label for='serial_number'>Serial Number:</label>
        <input type='text' name='serial_number' id='serial_number' class='form-control'></div>";

    $output .= "<div class='form-group'><label for='condition_desc'>Condition:</label>
        <textarea name='condition_desc' id='condition_desc' class='form-control'></textarea></div>";

    $output .= "<div class='form-group'><label for='category'>Category:</label>
        <select name='category' id='category' required class='form-control'>
            <option value='Standard'>Standard Equipment</option>
            <option value='Trailer'>Trailer</option>
            <option value='Tractors & Mowers'>Tractors & Mowers</option>
            <option value='Tools & Small Gear'>Tools & Small Gear</option>
        </select>
    </div>";

    $output .= "<div class='form-group'><label for='asking_price'>Asking Price ($):</label>
        <input type='number' name='asking_price' id='asking_price' step='0.01' required class='form-control'></div>";

    $output .= "<div class='form-group'><label for='min_price'>Minimum Acceptable Price ($):</label>
        <input type='number' name='min_price' id='min_price' step='0.01' required class='form-control'></div>";

    // Rental + Trade flags
    $output .= "<div class='form-group'><label for='rental_authorized'>Rental Authorized:</label>
        <select name='rental_authorized' id='rental_authorized' class='form-control'>
            <option value='0'>No</option>
            <option value='1'>Yes</option>
        </select></div>";

    $output .= "<div class='form-group'><label for='trade_authorized'>Trade Authorized:</label>
        <select name='trade_authorized' id='trade_authorized' class='form-control'>
            <option value='0'>No</option>
            <option value='1'>Yes</option>
        </select></div>";

    // Notes
    $output .= "<div class='form-group'><label for='notes'>Notes:</label>
        <textarea name='notes' id='notes' class='form-control'></textarea></div>";

    // Pickup/Delivery Info
    $output .= "<div class='form-group'><label>Pickup Option:</label>
        <div class='form-check'>
            <input class='form-check-input' type='checkbox' name='pickup_required' id='pickup_required'>
            <label class='form-check-label' for='pickup_required'>
                Free Pickup (within 30 miles, 16ft trailer max, profit ≥ \$75)
            </label>
        </div></div>";
...
$content .= "<div class='h6 text-muted'>Add consignor first</div>";
...
$content .= "<table class='table table-striped'>";
$content .= "<thead><tr><th>Item</th><th>Consignor</th><th>Category</th><th>Asking Price</th><th>Days on Lot</th><th>Agreement</th><th>Actions</th></tr></thead><tbody>";
...
// ðŸš¨ Insert abandonment check HERE
...
$rental_authorized = isset($_POST['rental_authorized']) ? (int)$_POST['rental_authorized'] : 0;
$trade_authorized = isset($_POST['trade_authorized']) ? (int)$_POST['trade_authorized'] : 0;
$notes = $_POST['notes'] ?? '';
...
$pickup_canceled_reason = $_POST['pickup_canceled_reason'] ?? null;
...
if ($id > 0) {
    ...
    $stmt = $conn->prepare($sql);
    $stmt->bind_param(
        "sssssddiissiissdisssi",  // ✅ Fixed bind string (21 types)
        $description, $make_model, $serial_number, $condition_desc,
        $category, $asking_price, $min_price, $consignor_id,
    ...
    $stmt = $conn->prepare($sql);
    $stmt->bind_param(
        "sssssdidsiissiiissdisss",  // ✅ Fixed bind string (22 types)
        $description, $make_model, $serial_number, $condition_desc, $category,
        $asking_price, $min_price, $consignor_id, $date_received,
...
if (isset($_GET['page']) && $_GET['page'] === 'item_details' && isset($_GET['item_id'])) {
    ...
    $content = ''; // Define local content for early output
    ...
    if ($item) {
        ...
        $content .= "<p><strong>Category:</strong> " . htmlspecialchars($item['category']) . "</p>";
        $content .= "<p><strong>Condition:</strong> " . nl2br(htmlspecialchars($item['condition_desc'])) . "</p>";
        ...
        $content .= "<p><strong>Status:</strong> " . htmlspecialchars($item['status']) . "</p>";
        $content .= "<p><strong>Rental Authorized:</strong> " . ($item['rental_authorized'] ? "Yes" : "No") . "</p>";
        $content .= "<p><strong>Trade Authorized:</strong> " . ($item['is_trade_authorized'] ? "Yes" : "No") . "</p>";
        $content .= "<p><strong>Notes:</strong><br>" . nl2br(htmlspecialchars($item['notes'])) . "</p>";
        ...
        $content .= "<hr><h4>Pickup Info</h4>";
        $content .= "<p><strong>Pickup Phone:</strong> " . htmlspecialchars($item['pickup_phone']) . "</p>";
        $content .= "<p><strong>Pickup Address:</strong> " . htmlspecialchars($item['pickup_address']) . "</p>";
        $content .= "<p><strong>Scheduled Pickup:</strong> " . $formatted_pickup . "</p>";
        $content .= "<p><strong>One-Way Mileage:</strong> " . htmlspecialchars($item['mileage']) . "</p>";
        ...
        $content .= "</div>";
        $content .= "<a href='?page=inventory' class='btn btn-secondary mt-3'>Back to Inventory</a>";
    } else {
        $content .= "<div class='alert alert-danger'>Item not found.</div>";
    }
...
if ($action === 'update_item' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $condition_desc = $conn->real_escape_string($_POST['condition_desc']);
    $category = $conn->real_escape_string($_POST['category']);
    $asking_price = floatval($_POST['asking_price']);
    $min_price = floatval($_POST['min_price']);
    $consignor_id = $conn->real_escape_string($_POST['consignor_id']);
    $rental_authorized = isset($_POST['rental_authorized']) ? (int) $_POST['rental_authorized'] : 0;
    $trade_authorized = isset($_POST['trade_authorized']) ? (int) $_POST['trade_authorized'] : 0;
    $status = $conn->real_escape_string($_POST['status']);
    $notes = $conn->real_escape_string($_POST['notes']);
    $owned_by_company = ($consignor_id === 'house_inventory' || $consignor_id === '' || $consignor_id === null) ? 1 : 0;

    // Pickup
    $pickup_phone = $conn->real_escape_string($_POST['pickup_phone'] ?? '');
    ...
    $pickup_canceled = isset($_POST['cancel_pickup']) ? 1 : 0;
    $pickup_canceled_reason = $conn->real_escape_string($_POST['pickup_canceled_reason'] ?? '');
    $pickup_canceled_at = $pickup_canceled ? "'" . date('Y-m-d H:i:s') . "'" : 'NULL';

    // ✅ Build and run SQL manually
    $sql = "
        UPDATE items SET
            ...
            min_price = $min_price,
            consignor_id = " . ($consignor_id === 'house_inventory' ? 'NULL' : (int)$consignor_id) . ",
            rental_authorized = $rental_authorized,
            is_trade_authorized = $trade_authorized,
            status = '$status',
            notes = '$notes',
            ...
            pickup_canceled_reason = '$pickup_canceled_reason',
            pickup_canceled_at = $pickup_canceled_at,
...
if ($action === 'take_ownership' && isset($_GET['item_id']) && is_numeric($_GET['item_id'])) {
    $conn = connectDB();
    $item_id = (int)$_GET['item_id'];
    ...
    if ($result && $result->num_rows > 0) {
        ...
        if (!$insert->execute()) {
            echo "<div class='alert alert-danger'>Failed to insert abandonment record: " . $insert->error . "</div>";
            exit;
        }
        ...
        if (!$update->execute()) {
            echo "<div class='alert alert-danger'>Failed to update item: " . $update->error . "</div>";
            exit;
        }

        $_SESSION['success_message'] = "✅ Item successfully taken into house inventory.";
    } else {
        $_SESSION['error_message'] = "⚠️ Item not found.";
    }
...
// =======================[ ACTION: generate_blank_consignor agreement ]=======================
...
if (isset($action) && $action ==='generate_blank_agreement') {
 ...
 $pdf->Write(0, "Back2Work Equipment Consignment Agreement", '', 0, 'C', true);
 $pdf->Ln(4);
 $pdf->Write(0, "Date: " . date('m/d/Y'), '', 0, 'L', true);
 $pdf->Ln(2);
 // Fill in if consignor_id is passed
 if (isset($_GET['consignor_id'])) {
     $conn = connectDB();
     $stmt = $conn->prepare("SELECT * FROM consignors WHERE id = ?");
     ...
     if ($result && $result->num_rows > 0) {
         $consignor = $result->fetch_assoc();
         $pdf->Write(0, "Consignor Name: " . $consignor['name'], '', 0, 'L', true);
         $pdf->Write(0, "Phone: " . $consignor['phone'], '', 0, 'L', true);
         $pdf->Write(0, "Email: " . $consignor['email'], '', 0, 'L', true);
         $pdf->Write(0, "Address: " . $consignor['address'], '', 0, 'L', true);
     }
     $conn->close();
 } else {
     $pdf->Write(0, "Consignor Name: _________________________", '', 0, 'L', true);
     $pdf->Write(0, "Phone: _________________________", '', 0, 'L', true);
     $pdf->Write(0, "Email: _________________________", '', 0, 'L', true);
     $pdf->Write(0, "Address: _________________________", '', 0, 'L', true);
 }
 $pdf->Ln(4);
...
// ===== [SALES SECTION] =====
// Record sale, sale agreement, payment tracking
// ====================[ SALES | function recordSale() ]====================
/**
 * Records a completed sale for an item.
 * - Calculates commission
 * - Stores license file (if provided)
 * - Applies store credit (if used)
 * - Updates item status and repeat seller flag
 * @param int $item_id
 * @param float $sale_price
 * @param string $buyer_name
 * @param string $buyer_contact
...
function recordSale($item_id, $sale_price, $buyer_name, $buyer_contact, $payment_method, $credit_applied = 0) {
    ...
    $sql_item = "SELECT i.*, c.repeat_seller
                 FROM items i
                 LEFT JOIN consignors c ON i.consignor_id = c.id
                 WHERE i.id = ?";
    ...
    // STEP 2: Check if consignor has multiple active items
    $multiple_items = 0;
    if (!empty($item['consignor_id'])) {
    ...
    // STEP 3: Calculate commission and tax
    $commission = calculateCommission($sale_price);
    $comm_rate = $commission['rate'];
    $comm_amount = $commission['amount'];
    $sales_tax = $sale_price * 0.0825;
    $today = date('Y-m-d');

    $notes = ($item['owned_by_company'] && $item['status'] === 'abandoned')
        ? 'House Inventory Sale (After Abandonment)'
        : NULL;

    // STEP 4: Handle license file upload
    $license_path = null;
    if (isset($_FILES['buyer_license']) && $_FILES['buyer_license']['error'] === UPLOAD_ERR_OK) {
        ...
        $filename = time() . "_" . basename($_FILES['buyer_license']['name']);
        $license_path = "uploads/licenses/" . $filename;
        move_uploaded_file($_FILES['buyer_license']['tmp_name'], $upload_dir . $filename);
    }

    // STEP 5: Insert sale record
    $sql_sale = "INSERT INTO sales (
        item_id, sale_date, sale_price, commission_rate, commission_amount, sales_tax,
        buyer_name, buyer_contact, payment_method, credit_applied, notes, license_file
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    
    $stmt_sale = $conn->prepare($sql_sale);
    $stmt_sale->bind_param(
        "isddddsssdds",
        $item_id, $today, $sale_price, $comm_rate, $comm_amount, $sales_tax,
        $buyer_name, $buyer_contact, $payment_method, $credit_applied, $notes, $license_path
    );
    ...
    if ($stmt_sale->execute()) {
        ...
        // Update item as sold
        $stmt_update = $conn->prepare("UPDATE items SET status = 'sold' WHERE id = ?");
        $stmt_update->bind_param("i", $item_id);
        $stmt_update->execute();

        // Mark consignor as repeat seller
        if (!empty($item['consignor_id'])) {
            $stmt_repeat = $conn->prepare("UPDATE consignors SET repeat_seller = 1 WHERE id = ?");
            $stmt_repeat->bind_param("i", $item['consignor_id']);
            $stmt_repeat->execute();
        }

        // Log redeemed store credit
        if ($credit_applied > 0) {
            $stmt_credit = $conn->prepare("INSERT INTO credits_redeemed (customer_name, amount, date_redeemed) VALUES (?, ?, ?)");
            $stmt_credit->bind_param("sds", $buyer_name, $credit_applied, $today);
            $stmt_credit->execute();
        }
...
// Handle actions
if ($action == 'process_sale' && isset($_GET['item_id']) && $_SERVER['REQUEST_METHOD'] == 'POST') {
    $item_id = intval($_GET['item_id']);  // Ensure it's numeric
...
if (isset($_FILES['buyer_license']) && $_FILES['buyer_license']['error'] === UPLOAD_ERR_OK) {
    ...
    // âœ… Buyer name logic - pick from dropdown OR typed input
    $buyer_name = $_POST['buyer_name_select'] ?? '';
    if (empty($buyer_name)) {
        $buyer_name = $_POST['buyer_name'] ?? 'House Inventory';
    }
    $buyer_contact = $_POST['buyer_contact'] ?? '';
    $payment_method = $_POST['payment_method'] ?? '';
    $credit_applied = floatval($_POST['credit_applied'] ?? 0);
    // âœ… Call recordSale with the new parameter
    $sale_id = recordSale($item_id, $sale_price, $buyer_name, $buyer_contact, $payment_method, $credit_applied);
    if ($sale_id) {
        // Immediately show the invoice content without redirect
        $content .= "<div class='alert alert-success'>Sale recorded successfully!</div>";
        $invoice = generateInvoice($sale_id);
        if ($invoice) {
            $content .= $invoice;
        } else {
            $content .= "<div class='alert alert-danger'>Invoice could not be generated.</div>";
        }
    } else {
        $content .= "<div class='alert alert-danger'>Error recording sale! Check the recordSale function.</div>";
    }
}
...
// ===== [RENTALS SECTION] =====
// Create rental, process returns, upload license, pricing
 ...
 if ($action == 'export_completed_rentals_pdf') {
    ...
    $html = "<h2>Completed Rentals</h2><table border='1' cellpadding='4'>
                <thead><tr>
...
// Export rental history PDF for a specific consignor
    ...
    $html = '<h2>Rental History</h2><table border="1" cellspacing="0" cellpadding="4" width="100%">';
...
// Page override: go to record_sale if triggered
...
$content .= "<div class='form-group'>
<label for='buyer_contact'>Buyer Contact (if new):</label>
<input type='text' name='buyer_contact' id='buyer_contact' class='form-control'>
</div>";
// Inside your 'Record Sale' form generation block:
$content .= "<div class='form-group mt-4'>";
$content .= "<label><strong>Pickup/Delivery Options:</strong></label><br>";
$content .= "<div class='form-check'>";
$content .= "  <input class='form-check-input' type='radio' name='delivery_option' id='pickup' value='pickup'>";
$content .= "  <label class='form-check-label' for='pickup'>Free Pickup (within 30 miles, 16ft trailer max, profit &ge; $75)</label>";
$content .= "</div>";
$content .= "<div class='form-check'>";
$content .= "  <input class='form-check-input' type='radio' name='delivery_option' id='delivery' value='delivery'>";
$content .= "  <label class='form-check-label' for='delivery'>Flat-Rate Delivery ($50) (within 30 miles, 16ft trailer max, profit &ge; $75)</label>";
$content .= "</div>";
$content .= "<div class='form-check'>";
$content .= "  <input class='form-check-input' type='radio' name='delivery_option' id='none' value='none' checked>";
$content .= "  <label class='form-check-label' for='none'>No Pickup/Delivery</label>";
$content .= "</div>";
$content .= "</div>";

// Date and Time Picker
$content .= "<div class='form-group mt-3'>";
$content .= "  <label for='scheduled_datetime'><strong>Schedule Date & Time:</strong></label>";
$content .= "  <input type='datetime-local' name='scheduled_datetime' id='scheduled_datetime' class='form-control'>";
$content .= "</div>";
$content .= "<div class='form-group'>
<label for='buyer_phone'>Buyer Phone:</label>
<input type='text' class='form-control' name='buyer_phone' id='buyer_phone' required>
</div>";
$content .= "<div class='form-group'>
<label for='buyer_address'>Buyer Address:</label>
<textarea class='form-control' name='buyer_address' id='buyer_address' rows='2' required></textarea>
</div>";
$content .= "<div class='form-group'>
<label for='delivery_method'>Delivery or Pickup:</label>
...
<label for='mileage'>Mileage (one way):</label>
<input type='number' step='0.1' class='form-control' name='mileage' id='mileage'>
<small class='form-text text-muted'>Must be within 30 miles</small>
</div>";
$content .= "<div class='form-group'>
<label for='scheduled_time'>Scheduled Pickup/Delivery Time:</label>
...
<input type='file' name='buyer_license' id='buyer_license' class='form-control-file' accept='image/*,.pdf' required>
<small class='form-text text-muted'>Valid, unexpired photo ID required for all sales. PDF or image only.</small>
</div>
<div class='form-check mt-2'>
<input type='checkbox' name='not_military_id' id='not_military_id' class='form-check-input' required>
...
let totalDue = salePrice + salesTax;
let balanceDue = totalDue - creditApplied;
let toConsignor = salePrice - commissionAmount;
let deliveryEligible = (salePrice - commissionAmount) >= 75;
let eligibilityIcon = deliveryEligible ? '? Meets delivery requirement' : '? Not eligible for delivery';
let eligibilityNote = deliveryEligible ? 'Meets delivery requirement' : 'Not eligible for delivery';
function formatMoney(n) {
return '$' + n.toFixed(2);
}
let html = '<p><strong>Sale Price:</strong> ' + formatMoney(salePrice) + '</p>';
html += '<p><strong>Commission (' + commissionRate + '%):</strong> ' + formatMoney(commissionAmount) + 
' <span title="' + eligibilityNote + '">' + eligibilityIcon + '</span></p>';
...
$conn->close();
        ...
        if ($action === 'process_rental' && isset($_GET['item_id'])) {
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                // --- VALIDATION ---
                if (empty($_POST['scheduled_pickup']) || empty($_POST['scheduled_return'])) {
                    $content .= "<div class='alert alert-danger'>Scheduled pickup and return are required.</div>";
                    return;
                }
        
                $daily_rate = (float) $_POST['daily_rate'];
                if ($daily_rate <= 0) {
                    $content .= "<div class='alert alert-danger'>Daily rate must be greater than zero.</div>";
                    return;
                }
        
                $renter_name = trim($_POST['renter_name']);
                $renter_contact = trim($_POST['renter_contact']);
                if (empty($renter_name) || empty($renter_contact)) {
                    $content .= "<div class='alert alert-danger'>Renter name and contact person are required.</div>";
                    return;
                }
                ...
                $rental_start        = substr($scheduled_pickup, 0, 10); // 'YYYY-MM-DD'
                $rental_end          = substr($scheduled_return, 0, 10);
        
                $deposit_amount      = isset($_POST['deposit_amount']) ? floatval($_POST['deposit_amount']) : 0.00;
                $renter_phone        = trim($_POST['renter_phone']);
                $renter_email        = trim($_POST['renter_email']);
                $renter_address      = trim($_POST['renter_address']);
                $renter_id_number = trim($_POST['renter_id_number']);
                $license_state       = trim($_POST['license_state']);
                $pickup_required     = isset($_POST['pickup_required']) ? 1 : 0;
                $delivery_required   = isset($_POST['delivery_required']) ? 1 : 0;
                $scheduled_delivery  = $_POST['scheduled_delivery'] ?? null;
                $delivery_phone      = trim($_POST['delivery_phone'] ?? '');
                $delivery_address    = trim($_POST['delivery_address'] ?? '');
                $mileage             = isset($_POST['mileage']) && is_numeric($_POST['mileage']) ? floatval($_POST['mileage']) : null;
                ...
                if ($delivery_required && $mileage <= 30 && $total_amount >= 75) {
                    $delivery_fee = 50.00;
                ...
                $stmt = $conn->prepare("INSERT INTO rentals (
                    ...
                    renter_id_number, license_state, license_file,
                    daily_rate, rental_start, rental_end,
                    pickup_required, delivery_required, delivery_phone, delivery_address,
                    mileage, scheduled_pickup, scheduled_return,
                    deposit_amount, total_amount, commission_rate, commission_amount,
                    status
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active')");
                ...
                $stmt->bind_param("isssssssssssiiisssddddd",
        $item_id, $renter_name, $renter_phone, $renter_email, $renter_address, $renter_contact,
        $renter_id_number, $license_state, $license_file,
        $daily_rate, $rental_start, $rental_end,
        $pickup_required, $delivery_required, $delivery_phone, $delivery_address,
        ...
        $deposit_amount, $total_amount, $commission_rate, $commission_amount
                ...
                if ($stmt->execute()) {
                    ...
                    header("Location: ?action=generate_rental_invoice&rental_id=" . $rental_id);
                    exit;
                } else {
                    $content .= "<div class='alert alert-danger'>Error saving rental: " . $conn->error . "</div>";
                }
        ...
        if ($action === 'print_return_receipt' && isset($_GET['rental_id']) && is_numeric($_GET['rental_id'])) {
            ...
            if ($result && $result->num_rows > 0) {
                ...
                echo "<style>body { font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto; padding: 40px; }</style>";
                echo "<h2>Rental Return Receipt</h2><hr>";
                ...
                if (!empty($rental['deposit_amount'])) {
                    echo "<p><strong>Deposit Collected:</strong> $" . number_format($rental['deposit_amount'], 2) . "</p>";
                }
                if (!empty($rental['deposit_refunded'])) {
                    echo "<p><strong>Deposit Refunded:</strong> $" . number_format($rental['deposit_refunded'], 2) . "</p>";
                }
        ...
        if ($action === 'end_rental' && isset($_GET['rental_id']) && is_numeric($_GET['rental_id'])) {
            ...
            if ($result && $result->num_rows > 0) {
                ...
                if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                    ...
                    $stmt = $conn->prepare("UPDATE rentals SET status = 'completed', inspection_passed = ?, inspection_notes = ?, returned_on = ?, inspection_file = ? WHERE id = ?");
                    $stmt->bind_param("isssi", $inspection_passed, $inspection_notes, $returned_on, $inspection_file, $rental_id);
        
                    if ($stmt->execute()) {
                        if ($make_available) {
                            $stmt_update = $conn->prepare("UPDATE items SET rental_authorized = 1 WHERE id = ?");
                            $stmt_update->bind_param("i", $rental['item_id']);
                            $stmt_update->execute();
                            $stmt_update->close();
                        }
                        header("Location: ?page=rentals&success=1");
                        exit;
                    } else {
                        $content .= "<div class='alert alert-danger'>Error updating rental: " . $conn->error . "</div>";
                    }
                    $stmt->close();
                }
                ...
                $content .= "<button type='submit' class='btn btn-success'>Complete Rental</button>";
                $content .= "</form>";
                $content .= "</div></div></div>";
            } else {
    ...
    if ($action === 'print_return_receipt' && isset($_GET['rental_id']) && is_numeric($_GET['rental_id'])) {
        ...
        if ($result && $result->num_rows > 0) {
            ...
            echo "Renter acknowledges that the equipment is in good working condition at the time of rental. Renter assumes full responsibility for any loss, damage, or repairs required. The card on file may be charged for repair or replacement of damaged or missing equipment.<br><br>";
    ...
    if (
        $action == 'generate_rental_invoice' &&
        isset($_GET['rental_id']) &&
        is_numeric($_GET['rental_id'])
    ) {
        $rental_id = (int) $_GET['rental_id'];
        $conn = connectDB();
        $stmt = $conn->prepare("SELECT r.*, i.description, i.make_model FROM rentals r LEFT JOIN items i ON r.item_id = i.id WHERE r.id = ?");
        ...
        if ($result && $result->num_rows > 0) {
            ...
            $subtotal = $days * floatval($data['daily_rate']);
            $deliveryFee = ($data['delivery_required'] && $data['mileage'] <= 30 && $subtotal >= 75) ? 50.00 : 0.00;
            ...
            $content .= "<tr><td>Sales Tax (8.25%)</td><td style='text-align: right;'>$" . number_format($tax, 2) . "</td></tr>";
            ...
            $content .= "Renter acknowledges that the equipment is in good working condition at the time of rental. Renter assumes full responsibility for any loss, damage, or repairs required. The card on file may be charged for repair or replacement of damaged or missing equipment.<br><br>";
...
if ($action === 'start_rental' && isset($_GET['item_id'])) {
    ...
    if ($result && $result->num_rows > 0) {
        ...
        $content .= "<form method='post' action='?action=save_rental'>
...
// ===== [TRADES SECTION] =====
// Trade contracts, trade logic, broker fee handling
// ====================[ TRADES | if ($action === 'trade_contract') ]====================
/**
 * Display a printable Trade Agreement Contract.
 * Triggered via: ?action=trade_contract&trade_id={id}
 */
// ====================[ ACTION: start_trade ]====================
if ($action === 'start_trade' && isset($_GET['item_id'])) {
    $content = ""; // Ensure $content is initialized
    // ... Your Start Trade logic goes here ...
    
    echo displayPage($content); // Safely wrap in the layout
    exit; // Stop other logic from continuing
}
...
if ($action === 'save_trade' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO trades (item_id, trade_for, trade_value, broker_fee, trade_notes, trader_name, trader_phone, trader_email, trader_address, trader_id, license_state, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
    $stmt->bind_param("isdssssssss", $item_id, $trade_for, $trade_value, $broker_fee, $trade_notes, $trader_name, $trader_phone, $trader_email, $trader_address, $trader_id, $license_state);

    if ($stmt->execute()) {
        markItemAsTraded($conn, $item_id);
...
if ($action === 'update_trade' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE trades SET item_id = ?, trade_for = ?, trade_value = ?, broker_fee = ?, trade_notes = ?, trader_name = ?, trader_phone = ?, trader_email = ?, trader_address = ?, trader_id = ?, license_state = ? WHERE id = ?");

    if ($stmt === false) {
        die("<div class='alert alert-danger'>Prepare failed: " . $conn->error . "</div>");
    }

    $stmt->bind_param("isdssssssssi", $item_id, $trade_for, $trade_value, $broker_fee, $trade_notes, $trader_name, $trader_phone, $trader_email, $trader_address, $trader_id, $license_state, $trade_id);
 ...
 if ($action === 'trade_contract' && isset($_GET['trade_id'])) {
    ...
    $stmt = $conn->prepare("
        SELECT t.*, i.description, i.make_model, i.asking_price,
               c.name AS consignor_name, c.phone AS consignor_phone
        FROM trades t
    ...
    if ($result && $result->num_rows > 0) {
        ...
        $net_value = number_format($trade['trade_value'] - $broker_fee_amt, 2);
        ...
        <style>
            /* [TRADES] Contract Print Styling */
            body { font-family: Arial, sans-serif; background-color: #f5f5f5; margin: 5; padding: 10px; }
            .contract-container { max-width: 500px; margin: 0 auto; background-color: #fff; padding: 25px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); border-radius: 4px; }
            h2, h4 { color: #333; border-bottom: 1px solid #ddd; }
            .terms { font-size: 14px; color: #333; word-wrap: break-word; white-space: normal; max-width: 100%; box-sizing: border-box; }
            table { width: 100%; margin-top: 30px; border-collapse: collapse; }
            table td { padding: 10px 5px; }
            .btn { padding: 8px 16px; background-color: #f0ad4e; color: white; border-radius: 4px; border: none; cursor: pointer; }
            .btn:hover { background-color: #ec971f; }
            @media print {
                body * { visibility: hidden; }
                .contract, .contract * { visibility: visible; }
                .contract { position: absolute; left: 0; top: 0; width: 100%; font-family: sans-serif; background-color: white; }
                .contract-container { box-shadow: none; padding: 0; }
                .d-print-none { display: none !important; }
                @page { size: auto; }
                body { margin: 0; padding: 0; font-size: 14pt; }
            }
        </style>";
    } else {
...
// ===== [PROMOTIONS SECTION] =====
// Credit offers, promo logic
// =======================[ ACTION: save_promotion ]=======================
if ($action === 'save_promotion' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    $conn = connectDB();
...
// ===== [SCHEDULING SECTION] =====
// Pickup/delivery, calendar, date assignments


// ===== [EXPORTS / REPORTS SECTION] =====
// CSV, PDF, tax reports, printable views
// ====================[ EXPORTS | function generateConsignmentAgreement() ]====================
/**
 * Generate a polished PDF consignment agreement for a given item.
 * Uses MyPDF class for layout and outputs inline to browser.
 * @param int $item_id The ID of the item being consigned
 * @return false If the item or consignor is not found
 */
function generateConsignmentAgreement($item_id) {
    $conn = connectDB();

    // Get item and consignor details
    $sql = "SELECT i.*, c.* FROM items i JOIN consignors c ON i.consignor_id = c.id WHERE i.id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $item_id);
    $stmt->execute();
    $result = $stmt->get_result();
...
8. PAYMENT TO CONSIGNOR
Payment will be issued based on the Consignor’s selected payment preference after the sale is completed.
...
// ====================[ EXPORTS | function generateInvoice() ]====================
/**
 * Generates a printable HTML invoice for a completed sale.
 * Returns invoice markup or fallback message if sale is not found.
 * @param int $sale_id The ID of the sale record
 * @return string HTML content for display or error
 */
function generateInvoice($sale_id) {
    ...
    if ($result && $result->num_rows > 0) {
        ...
        $invoice_content = "<div class='invoice-box'>";
        $invoice_content .= "<h1>BACK2WORK EQUIPMENT</h1>";
        $invoice_content .= "<h2>Sales Invoice</h2><hr>";
        $invoice_content .= "<p><strong>Invoice #:</strong> " . str_pad($data['id'], 5, '0', STR_PAD_LEFT) . "</p>";
        $invoice_content .= "<p><strong>Date:</strong> " . date('m/d/Y', strtotime($data['sale_date'])) . "</p>";
        $invoice_content .= "<p><strong>Sold To:</strong> " . htmlspecialchars($data['buyer_name']) . "</p>";
        $invoice_content .= "<p><strong>Item:</strong> " . htmlspecialchars($data['description']) . " (" . htmlspecialchars($data['make_model']) . ")</p>";
        $invoice_content .= "<p><strong>Serial Number:</strong> " . htmlspecialchars($data['serial_number']) . "</p>";
        $invoice_content .= "<hr>";
        $invoice_content .= "<p><strong>Sale Price:</strong> $" . number_format($data['sale_price'], 2) . "</p>";
        $invoice_content .= "<p><strong>Sales Tax (8.25%):</strong> $" . number_format($data['sales_tax'], 2) . "</p>";
        $invoice_content .= "<p><strong>Total Amount:</strong> $" . number_format($data['sale_price'] + $data['sales_tax'], 2) . "</p>";
        $invoice_content .= "<p><strong>Payment Method:</strong> " . htmlspecialchars($data['payment_method']) . "</p>";
        $invoice_content .= "<hr>";
        $invoice_content .= "<p><strong>Customer Signature:</strong> ______________________________________</p>";
        $invoice_content .= "<p><em>By signing, customer agrees that all sales are final.</em></p>";
        $invoice_content .= "<div style='text-align:center;margin-top:30px;'>";
        $invoice_content .= "<a href='?page=sales_history' class='btn btn-secondary'>View Sales to Print</a>";
        $invoice_content .= "</div>";
        $invoice_content .= "</div>";
    ...
    $conn->close();
    return "<p class='text-danger'>No sale found for invoice.</p>";
}


// ===== [FINANCE SECTION] =====
// Commissions, tax summaries, refunds
// ====================[ FINANCE | function calculateCommission() ]====================
/**
 * Calculates commission rate and amount based on a tiered price system.
 * Returns:
...
function calculateCommission($price) {
    ...
    return [
        ...
        'amount' => $price * $commission_rate
    ];
}



// ====================[ FINANCE | page: store_credit ]====================
/**
 * Displays a table of store credit balances grouped by customer.
 * Triggered via: ?page=store_credit
 */
...
if (
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $amount = $damage_fee + $delivery_fee + $fuel_surcharge + $cleaning_fee + $late_fee + $misc_fee;
        ...
        if ($result->num_rows === 0) {
            ...
            if ($issue_credit && !empty($customer_name)) {
                ...
                $stmtCredit->execute();
                $stmtCredit->close(); // Close the credit statement
            }

            $content .= "<div class='alert alert-success'>Refund successful." . ($issue_credit ? " Store credit issued to $customer_name." : "") . "</div>";
            $stmt->close(); // Close the insert statement
        }
        $infoStmt->close(); // Close the info statement
    }
    ...
    if ($result->num_rows > 0) {
        ...
        if ($type === 'sale') {
            ...
            $content .= "<li><strong>Renter:</strong> " . htmlspecialchars($transaction['renter_name']) . " (" . htmlspecialchars($transaction['renter_contact']) . ")</li>
                         ...
                         <li><strong>Total Amount:</strong> $" . number_format($transaction['total_amount'], 2) . "</li>";
        }
    
        $content .= "</ul></div></div>";
    
        $content .= "
<h2 class='mt-4'>Issue " . ucfirst($type) . " Refund</h2>
<form method='post' id='refundForm'>
    <div class='form-group'>
        <label>Refund Amount </label>
       <input type='number' name='amount' id='refund_amount' class='form-control' step='0.01'" . ($type === 'rental' ? " readonly" : "") . ">
...
if ($type === 'rental') {
    ...
    <div class='form-group mt-4'>
        ...
        <div class='form-row mb-2'>
            <div class='col'>
                <label>Damage Deposit <small class='text-muted'>(Suggested: \$100)</small></label>
                <input type='number' step='0.01' name='damage_fee' class='form-control refund-charge'>
            </div>
            <div class='col'>
                <label>Delivery/Pickup <small class='text-muted'>(Suggested: \$50)</small></label>
                <input type='number' step='0.01' name='delivery_fee' class='form-control refund-charge'>
            </div>
        </div>
        <div class='form-row mb-2'>
            <div class='col'>
                <label>Fuel Surcharge <small class='text-muted'>(Suggested: \$25)</small></label>
                <input type='number' step='0.01' name='fuel_surcharge' class='form-control refund-charge'>
            </div>
            <div class='col'>
                <label>Cleaning Fee <small class='text-muted'>(Suggested: \$50)</small></label>
                <input type='number' step='0.01' name='cleaning_fee' class='form-control refund-charge'>
            </div>
        </div>
        <div class='form-row mb-2'>
            <div class='col'>
                <label>Late Fee <small class='text-muted'>(Suggested: 1.5× daily rate)</small></label>
                <input type='number' step='0.01' name='late_fee' class='form-control refund-charge'>
            </div>
            <div class='col'>
                <label>Misc. Charge <small class='text-muted'>(Optional)</small></label>
                <input type='number' step='0.01' name='misc_fee' class='form-control refund-charge'>
            </div>
        </div>
    </div>";
}
        ...
        // Add JavaScript AFTER the form is closed, not inside the form
        if ($type === 'rental') {
            $content .= '
<script>
document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateRefundTotal() {
        ...
        document.getElementById("refund_breakdown").innerText = breakdown.join(", ");
    }
...
if ($action === 'redeem_credit' && isset($_GET['customer'])) {
    ...
    $content .= "    <div class='form-group'>";
    $content .= "        <label>Amount to Redeem</label>";
    $content .= "        <input type='number' name='amount' step='0.01' class='form-control' required>";
    $content .= "    </div>";
    $content .= "    <div class='form-group'>";
    $content .= "        <label>Redeemed By</label>";
    $content .= "        <input type='text' name='redeemed_by' class='form-control' required>";
    $content .= "    </div>";
    $content .= "    <button type='submit' class='btn btn-primary'>Submit</button>";
    $content .= "    <a href='?page=credits_summary' class='btn btn-secondary'>Cancel</a>";
    $content .= "</form>";
}
$conn->close();
} // end if ($action === 'redeem_credit')



// ====================[ UTILITIES / GENERAL FUNCTIONS ]====================
function markItemAsTraded($conn, $item_id) {
...
<body>
    <div class="container">
        <nav class="navbar navbar-expand-lg navbar-dark bg-dark border-bottom mb-3">
            <a class="navbar-brand font-weight-bold text-white" href="?page=dashboard">Back2Work Equipment</a>
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav">
                    <li class="nav-item"><a class="nav-link text-white'.($current_page == 'dashboard' ? ' active' : '').'" href="?page=dashboard">Dashboard</a></li>
                    <li class="nav-item"><a class="nav-link text-white'.($current_page == 'inventory' ? ' active' : '').'" href="?page=inventory">Inventory</a></li>
                    <li class="nav-item"><a class="nav-link text-white'.($current_page == 'consignors' ? ' active' : '').'" href="?page=consignors">Consignors</a></li>
                    <li class="nav-item"><a class="nav-link text-white'.($current_page == 'sales_history' ? ' active' : '').'" href="?page=sales_history">Sales</a></li>
                    <li class="nav-item"><a class="nav-link text-white'.($current_page == 'rentals' ? ' active' : '').'" href="?page=rentals">Rentals</a></li>
                    <li class="nav-item"><a class="nav-link text-white'.($current_page == 'aging_inventory' ? ' active' : '').'" href="?page=aging_inventory">Aging Items</a></li>
                    <li class="nav-item"><a class="nav-link text-white'.($current_page == 'promotions' ? ' active' : '').'" href="?page=promotions">Promotions</a></li>
                    <li class="nav-item"><a class="nav-link text-white'.($current_page == 'trade_log' ? ' active' : '').'" href="?page=trade_log">Trade Report</a></li>
                    <li class="nav-item"><a class="nav-link text-white'.($current_page == 'tax_report' ? ' active' : '').'" href="?page=tax_report">Tax Summary</a></li>
                </ul>
            </div>
        </nav>
...
// Main execution
// Fallback only if nothing else handled the request
