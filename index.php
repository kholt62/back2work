...
if (($action === 'print_return_receipt' || (isset($_GET['page']) && $_GET['page'] === 'print_return_receipt')) && isset($_GET['rental_id']) && is_numeric($_GET['rental_id'])) {
    ...
    if ($result && $result->num_rows > 0) {
        ...
        $total_without_deposit = $rental_fee + $additional_charges - $simple_refund;
        ...
        if ($deposit > 0) {
            ...
            if ($deposit_returned > 0 || $external_refund) {
                $refund_amount = $external_refund ? floatval($external_refund['amount']) : $deposit_returned;
    ...
    if (!$stmt) {
        ...
        if (!$result) {
            ...
            if ($rental['status'] === 'completed') {
                ...
                // Add refund fields if amount > 0
                if ($refund_amount > 0) {
                    $update_sql .= ", refund_amount = ?, refund_reason = ?, refund_date = ?";
                    $params[] = $refund_amount;
                    $params[] = $refund_reason;
                    $params[] = $refund_date;
                    $types .= "dss"; // Double, String, String
                }
                ...
                if (!$update_stmt) {
                    ...
                    if (!$update_stmt_bind_param($update_stmt, $types, $params)) {
                        ...
                        if (!$update_stmt->execute()) {
                            ...
                            if ($make_available) {
                                $item_stmt = $conn->prepare("UPDATE items SET rental_authorized = 1 WHERE id = ?");
                                if (!$item_stmt) {
                ...
                $content .= "<div class='card mb-3'>";
                $content .= "<div class='card-header'><strong>Quick Refund</strong> <small class='text-muted'>(for minor adjustments only)</small></div>";
                $content .= "<div class='card-body'>";
                $content .= "<div class='alert alert-warning'>
...
if ($action === 'issue_refund' && isset($_GET['type']) && isset($_GET['id'])) {
    ...
    if (!$stmt) {
        ...
        if ($result->num_rows > 0) {
            ...
            if ($type === 'sale') {
                ...
                if (isset($transaction['deposit']) && floatval($transaction['deposit']) > 0) {
                    ...
                    if ($deposit_returned > 0) {
                        ...
                        $content .= "<li><strong>Remaining Available:</strong> $" . number_format($available_deposit, 2) . "</li>";
                    }
                }
            }
            ...
            if ($type === 'rental' && isset($transaction['deposit']) && floatval($transaction['deposit']) > 0) {
                ...
                if ($available_deposit > 0) {
                    ...
                    if ($deposit_returned > 0) {
                        ...
                        $content .= "Available for Refund: $" . number_format($available_deposit, 2) . "</p>";
                    }
                    ...
                    $content .= "<div class='alert alert-warning'>
                        ...
                        " has already been returned. Any additional refund will need to be processed separately.
                    </div>";
                }
            }
            ...
            if ($type === 'rental' && isset($available_deposit) && $available_deposit > 0) {
                $content .= '
    <script>
    document.addEventListener("DOMContentLoaded", function () {
        const fields = [
        ...
        function updateRefundTotal() {
            ...
            // Calculate refund as deposit minus fees
            const refundAmount = Math.max(0, depositAmount - totalFees);
            document.getElementById("refund_amount").value = refundAmount.toFixed(2);
            
            // Show breakdown
            let breakdownText = "";
            if (depositAmount > 0) {
                breakdownText += "Available Deposit: $" + depositAmount.toFixed(2);
