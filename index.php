...
function recordSale($item_id, $sale_price, $buyer_name, $buyer_contact, $payment_method, $credit_applied = 0) {
    ...
    // Sanitize inputs manually for direct query
    $item_id = (int) $item_id;
    $sale_price = (float) $sale_price;
    $buyer_name = $conn->real_escape_string($buyer_name);
    $buyer_contact = $conn->real_escape_string($buyer_contact);
    ...
    // INSERT using direct query instead of prepared statement
    ...
    error_log("SQL Query: " . $sql_sale); // Log the exact SQL query
    
    if ($conn->query($sql_sale)) {
        $sale_id = $conn->insert_id;
        
        // Update item status
        $conn->query("UPDATE items SET status = 'sold' WHERE id = $item_id");
        
        // Update consignor as repeat seller (ONLY if consignor exists)
        if (!empty($item['consignor_id'])) {
            $consignor_id = (int) $item['consignor_id'];
            $conn->query("UPDATE consignors SET repeat_seller = 1 WHERE id = $consignor_id");
        }
        
        // Deduct used store credit
        if ($credit_applied > 0) {
            $sql_credit = "INSERT INTO credits_redeemed (customer_name, amount, date_redeemed) 
                          VALUES ('$buyer_name', $credit_applied, '$today')";
        ...
        error_log("SQL Error: " . $conn->error); // Log any SQL error
    }
...
function generateInvoice($sale_id) {
    ...
    if ($result && $result->num_rows > 0) {
        ...
        // Delivery details (if applicable)
        $invoice .= "<hr>";
        $invoice .= "<p><strong>Delivery Method:</strong> " . (!empty($data['delivery_method']) ? ucfirst(htmlspecialchars($data['delivery_method'])) : 'None') . "</p>";
        $invoice .= "<p><strong>Mileage (one-way):</strong> " . floatval($data['mileage']) . " miles</p>";
        ...
        if (!empty($data['scheduled_time']) && $data['scheduled_time'] !== '0000-00-00 00:00:00') {
            ...
            $invoice .= "<p><strong>Scheduled Time:</strong> N/A</p>";
        }
...
if ($action == 'process_sale' && isset($_GET['item_id']) && $_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    foreach ($_FILES as $key => $value) {
        $debug_log .= "$key: " . print_r($value, true) . "\n";
    }
    error_log($debug_log);
    
    $item_id = intval($_GET['item_id']);  // Ensure it's numeric
    $sale_price = floatval($_POST['sale_price'] ?? 0);  // THIS LINE WAS MISSING
    
    // Correctly save delivery method and fee
    $delivery_method = $_POST['delivery_method'] ?? '';
    $delivery_fee = ($delivery_method === 'delivery') ? 50.00 : 0.00;
    $mileage = floatval($_POST['mileage'] ?? 0);
    $scheduled_time = !empty($_POST['scheduled_time']) ? $_POST['scheduled_time'] : null;
    ...
    // Buyer name logic - pick from dropdown OR typed input
    ...
    // Call recordSale with the updated parameters including delivery information
    $sale_id = recordSale($item_id, $sale_price, $buyer_name, $buyer_contact, $payment_method, $credit_applied);
    
    if ($sale_id) {
        // Update the sales record with additional delivery information
        $conn = connectDB();
        ...
        // Debug: Log the values being used in the update query
        error_log("UPDATE Query Values: Method=$delivery_method, Fee=$delivery_fee, Mileage=$mileage, Time=$scheduled_time, SaleID=$sale_id");
        
        if ($stmt->execute()) {
            error_log("UPDATE Query Success: Rows affected: " . $stmt->affected_rows);
        } else {
        ...
        $verify_sql = "SELECT delivery_method, delivery_fee, mileage, scheduled_time FROM sales WHERE id = ?";
        $verify_stmt = $conn->prepare($verify_sql);
        $verify_stmt->bind_param("i", $sale_id);
        $verify_stmt->execute();
        $verify_result = $verify_stmt->get_result();
        $verify_data = $verify_result->fetch_assoc();
        error_log("Data verification after update: " . print_r($verify_data, true));
        $verify_stmt->close();
        
        $conn->close();
        
        // Display success and invoice
